...
// Track globally fetched endpoint sets to prevent duplicate fetches
 ...
 * Uses global caching to prevent duplicate fetches across components
 */
/**
 * Convenience hook to provide endpoint config functionality
 * Wraps the multiple endpoints hook for easier use
 */
export function useEndpointConfig() {
  const checkEndpoints = async (endpoints: string[]) => {
    // This will be handled by useMultipleEndpointsEnabled
    console.debug('[useEndpointConfig] Checking endpoints:', endpoints);
  };

  const isEndpointEnabled = (endpoint: string): boolean => {
    // Default to true if not in cache (optimistic)
    return globalEndpointCache[endpoint] !== false;
  };

  return {
    endpointStatus: globalEndpointCache,
    loading: false,
    error: null,
    checkEndpoints,
    isEndpointEnabled,
...
export function useMultipleEndpointsEnabled(endpoints: string[]): {
  ...
  const fetchAllEndpointStatuses = async (force = false) => {
    ...
    if (!hasJwt && isLoginPage) {
      console.debug('[useEndpointConfig] On login page without JWT - optimistically enabling all endpoints');
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          const optimisticStatus = endpoints.reduce((acc, endpoint) => {
            acc[endpoint] = true;
            globalEndpointCache[endpoint] = true;
            return acc;
          }, {} as Record<string, boolean>);
          setEndpointStatus(optimisticStatus);
          setLoading(false);
      ...
      // Get all requested endpoints from cache
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
      setError(errorMessage);
      console.error('[useEndpointConfig] Failed to check multiple endpoints:', err);
